...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 18 00:33:01 2018
...
# Fit the Kernel SVM classifier to the train set
...
plot.title("Kernel SVR Classifier (Train Set)")
plot.xlabel("Age")
...
for i,j in enumerate(np.unique(y_set)):
    plot.scatter(X_set[y_set==j,0], X_set[y_set==j,1], c=ListedColormap(("red","green"))(i), label=j)
plot.title("Kernel SVR Classifier (Test Set)")
plot.xlabel("Age")
plot.ylabel("Estimated Salary")
plot.legend()
plot.show()
